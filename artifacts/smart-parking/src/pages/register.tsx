...
export default function Register() {
  ...
  const onSubmit = (data: z.infer<typeof formSchema>) => {
    registerMutation.mutate({ data }, {
      onSuccess: (res) => {
        setToken(res.token);
        toast({ title: "Account created!", description: "Welcome to SmartPark." });
        setLocation("/dashboard");
      },
      onError: (err: any) => {
        toast({ 
          title: "Registration failed", 
          description: err.message || err.data?.error || "Could not create account.", 
          variant: "destructive" 
        });
      }
    });
    ...
    <div className="min-h-[100dvh] bg-background flex items-center justify-center p-4 relative overflow-hidden">
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Join SmartPark</h1>
          <p className="text-muted-foreground mt-2 text-center">Start finding spots faster.</p>
        </div>
